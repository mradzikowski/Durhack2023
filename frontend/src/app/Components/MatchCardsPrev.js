...
export default function MatchCardsPrev({ setSelected }) {
  ...
  const getTeams = async () => {
    const teams = await axios.get(
      "https://footballapi.pulselive.com/football/compseasons/489/teams",
    );
    setTeams(teams.data);
  };
  ...
  const getFixtures = async () => {
    ...
    const fixtures = await axios.get(
      `https://footballapi.pulselive.com/football/fixtures?comps=1&compSeasons=489&teams=${tString}&page=0&pageSize=200&sort=desc&statuses=C&altIds=true`
    );
